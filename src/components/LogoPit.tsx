...
// Ballpit — React Bits (https://www.reactbits.dev/backgrounds/ballpit)
// Component inspired by Kevin Levron: https://x.com/soju22/status/1858925191671271801
//
// Note: three r160 ships `Timer` in addons (not core), so that one import is
// adjusted; everything else is verbatim from the React Bits source.
...
// Replaces Ballpit's instanced sphere mesh: a field of camera-facing Sprites,
// each showing a brand-logo chip, driven by the same physics (W) as Ballpit.
class LogoField extends m {
  constructor(tools, config) {
    super();
    this.config = config;
    this.textures = tools.map(buildLogoTexture);
    this.physics = new W(config);
    this.sprites = [];
    for (let i = 0; i < config.count; i++) {
      const tex = this.textures[i % tools.length];
  ...
  update(e) {
    ...
    const { positionData, sizeData } = this.physics;
    for (let idx = 0; idx < this.sprites.length; idx++) {
      const sp = this.sprites[idx];
      _v.fromArray(positionData, 3 * idx);
      sp.position.copy(_v);
      if (idx === 0) { sp.visible = false; continue; } // index 0 = invisible cursor-follow influencer (no "logo cursor")
      sp.visible = true;
      const s = sizeData[idx] * 1.9;
      sp.scale.set(s, s, s);
    }
  }
  burst() {
    const cfg = this.config;
    const old = cfg.maxVelocity;
    cfg.maxVelocity = 0.6; // allow a lively burst (physics clamps velocity per frame)
    const v = this.physics.velocityData;
    for (let i = 1; i < cfg.count; i++) {
      const b = 3 * i;
      v[b] += (Math.random() - 0.5) * 0.8;
      v[b + 1] += Math.random() * 0.9 + 0.3; // upward pop
      v[b + 2] += (Math.random() - 0.5) * 0.2;
    }
  ...
  dispose() {
    clearTimeout(this._bt);
    this.sprites.forEach((sp) => sp.material.dispose());
    this.textures.forEach((t) => t.dispose());
  }
}

function createLogoPit(canvas, tools, props = {}) {
  const i = new x({
    canvas,
    size: "parent",
  ...
  const h = S({
    domElement: canvas,
    onMove() {
      ray.setFromCamera(h.nPosition, i.camera);
      i.camera.getWorldDirection(plane.normal);
      ray.ray.intersectPlane(plane, hit);
      field.physics.center.copy(hit);
      field.config.controlSphere0 = true;
    },
    onLeave() {
      field.config.controlSphere0 = false;
    },
    onClick() {
      field.burst();
    },
  ...
  function initialize(cfg) {
    if (field) {
      i.scene.remove(field);
      field.dispose();
    }
    field = new LogoField(tools, cfg);
    i.scene.add(field);
  }

  i.onBeforeRender = (e) => {
    if (!paused) field.update(e);
  };
  i.onAfterResize = (e) => {
    field.config.maxX = e.wWidth / 2;
    field.config.maxY = e.wHeight / 2;
  };
...
const LogoPit = ({
  ...
  useEffect(() => {
    ...
    if (!container || tools.length === 0) return;

    // Fresh canvas per mount (React StrictMode dev remounts forceLose the GL
    // context on a reused canvas).
    ...
    try {
      instanceRef.current = createLogoPit(canvas, tools, { followCursor, ...props });
    } catch (err) {
      console.error("[LogoPit] init failed:", err);
    }
