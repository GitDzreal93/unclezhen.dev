...
import { createApiToken, deleteApiToken, revokeApiToken, updateApiTokenScopes } from "@/lib/admin";
...
export default function ApiTokenManager({ tokens, locale, documentation }: { tokens: Token[]; locale: Locale; documentation: Documentation }) {
  ...
  function saveEdit(id: string) {
    ...
    startTransition(async () => {
      try {
        ...
        fd.set("id", id);
        editScopes.forEach((s) => fd.append("scopes", s));
        await updateApiTokenScopes(fd);
        setEditingId(null);
        router.refresh();
      } catch (e) {
  ...
  return <>
    ...
    {error && <p className="admin-login__err" style={{ margin: "0 16px" }}>{error}</p>}
      ...
      <div className="api-workspace__tokens">
        ...
        <section className="settings-card">
          ...
          <div className="api-token-list">{tokens.length === 0 ? <p className="hint">还没有 Token。</p> : tokens.map((token) => {
            ...
            const editable = !token.revokedAt && !expired;
            const isEditing = editingId === token.id;
            return (
              <article className="api-token-row" key={token.id}>
                <div>
                  <strong>{token.name}</strong>
                  <div className="mono muted">{token.prefix}••••</div>
                  <div className="scope-chips">{token.scopes.map((scope) => <code key={scope}>{scope}</code>)}</div>
                </div>
                <div className="api-token-meta">
                  <span className={status === "有效" ? "token-status token-status--active" : "token-status"}>{status}</span>
                  <span>过期：{fmt(token.expiresAt)}</span>
                  <span>最后使用：{fmt(token.lastUsedAt)}</span>
                </div>
                <div className="api-token-actions">
                  {editable && !isEditing && <button className="btn btn--ghost btn--sm" disabled={pending} onClick={() => startEdit(token)}>权限</button>}
                  {editable && <button className="btn btn--ghost btn--sm" disabled={pending} onClick={() => action(revokeApiToken, token.id, "确定撤销此 Token？撤销后无法恢复。")}>撤销</button>}
                  <button className="btn btn--ghost btn--sm" disabled={pending} onClick={() => action(deleteApiToken, token.id, "永久删除此 Token？审计记录会保留，但不能恢复。")}>删除</button>
                </div>
                {isEditing && (
                  <div className="api-token-edit">
                    <fieldset className="api-token-scopes">
                      <legend>编辑「{token.name}」的权限</legend>
                      {API_SCOPES.map((scope) => (
                        <label key={scope}>
                          <input type="checkbox" checked={editScopes.includes(scope)} onChange={() => toggleScope(scope)} disabled={pending} />
                          {" "}<code>{scope}</code>
                        </label>
                      ))}
                    </fieldset>
                    <div className="admin-actions">
                      <button className="btn btn--primary btn--sm" type="button" disabled={pending} onClick={() => saveEdit(token.id)}>保存权限</button>
                      <button className="btn btn--ghost btn--sm" type="button" disabled={pending} onClick={() => { setEditingId(null); setError(""); }}>取消</button>
                    </div>
                  </div>
                )}
              </article>
